...
## Deploy
The repository contains the complete Supabase migration history used by the live project plus the `arena-control` Edge Function source. The live Supabase project is already provisioned and `arena-control` is deployed. Push this repository to Vercel. Vercel injects `VERCEL_OIDC_TOKEN` automatically, so production does not require a Supabase service-role environment variable.
